...
Last post showed the steps of one HTTP request. Any of them can hang.
...
Key properties
→ Most HTTP clients have two: a connect timeout (to open the connection) and a read timeout (to wait for the answer).
...
→ A timeout does not mean the work failed. The server may still finish it. (More on that in #06.)
